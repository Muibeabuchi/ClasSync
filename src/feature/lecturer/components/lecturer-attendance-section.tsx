...
interface LecturerAttendanceSectionProps {
  courseId: string;
  onBack: () => void;
}
...
const LecturerAttendanceSection = ({
  //   courseId,
  onBack,
}: LecturerAttendanceSectionProps) => {
      ...
      interval = setInterval(() => {
        setRemainingTime((prev) => {
          if (prev <= 1) {
            ...
            toast.info(
              'Attendance Session Ended , The attendance session has automatically ended.',
            );
            return 0;
          }
          return prev - 1;
  ...
  const startAttendanceSession = () => {
    ...
    toast.info(
      `Attendance Session Started Session will run for ${sessionDuration} minutes`,
    );
  };
  ...
  const endAttendanceSession = () => {
    ...
    setRemainingTime(0);
    toast.info(
      'Attendance Session Ended The attendance session has been manually ended.',
    );
  };
